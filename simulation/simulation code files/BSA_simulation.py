...
for i in range(0,300, 100):
    ...
    # Run BSA 
    best_solution_pso = bee_search_algorithm(renewable_capacity, conventional_capacity, data, 1000)

    solar_output_bsa, wind_output_bsa, coal_output_bsa, gas_output_bsa = best_solution_pso
    ...
    optimal_allocation = {
        'SOLAR': solar_output_bsa,
        'WIND': wind_output_bsa,
        'COAL': coal_output_bsa,  # Using only 8074.4 MW even though 12000 MW is available
        'GAS': gas_output_bsa   # Using only 9920.94 MW even though 10000 MW is available
    }

    # Maximum capacity for coal and gas (not to be fully used)
    max_capacity = {
        'COAL': 12000,  # Max possible
        'GAS': 10000    # Max possible
    }
    ...
    def allocate_power():
        ...
        coal_surplus = round(max_capacity['COAL'] - optimal_allocation['COAL'], 2)
        gas_surplus = round(max_capacity['GAS'] - optimal_allocation['GAS'], 2)
    ...
    total_cost_bsa = calculate_total_cost(min(availability['SOLAR'],optimal_allocation['SOLAR']) , min(availability['WIND'],optimal_allocation['WIND']),availability['COAL'], availability['GAS'])
    ...
    'total_cost': round(total_cost_bsa,2),
    **frequency_stats  
    }
...
np.save("BSA_frequency_data.npy", all_frequencies)    
